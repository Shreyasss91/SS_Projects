...
> **Depth-level reality (verified against OpenAlgo/FYERS source).** OpenAlgo's default depth is **5 levels**; deeper books (20/30/50) are *broker-capability dependent* (`websocket_proxy/mapping.py::get_supported_depth_levels`, `base_adapter.py:254`). True **50-level** depth is available **only via FYERS' separate tick-by-tick (TBT) WebSocket**, and the FYERS adapter restricts TBT to `TBT_SUPPORTED_EXCHANGES = {"NSE", "NFO"}` (`broker/fyers/streaming/fyers_websocket_adapter.py:44`). Practical consequence: **NIFTY** options (exchange **NFO**) get true 50-level depth, but **SENSEX** options trade on **BFO**, which is *not* TBT-supported and therefore degrade to **5-level** depth. The recorder must **auto-detect the actual depth level per symbol** and record whatever the feed delivers (best-effort), never assume 50. **Live-verified cap (2026-07-06, P9):** FYERS' TBT feed additionally limits **5 symbols per channel** (it exposes channels 1–50), and stock OpenAlgo pinned *every* 50-depth subscription to channel `"1"` (`broker/fyers/streaming/fyers_websocket_adapter.py`) — so only 5 symbols could carry 50-level depth at once and a large NIFTY chain silently starved (the broker returns `symbol count exceeds limit: 5` and stalls the channel). Capturing a full NIFTY chain at 50-level therefore requires the OpenAlgo **channel-spread patch** that buckets 5 symbols/channel across channels 1–50 (ceiling 250) — see `Documents/patches/OPENALGO_PATCH.md` and `Documents/patches/Phase9_notes.md`. See §3.3 for the `:50` subscription mechanism and §3.2.5 for the capability preflight.
