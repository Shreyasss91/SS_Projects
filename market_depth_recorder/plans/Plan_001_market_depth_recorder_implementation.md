...
> **Authoritative plan document.** Canonical location:
> `market_depth_recorder/plans/Plan_001_market_depth_recorder_implementation.md`.
> Moved here from `~/.claude/plans/refer-market-depth-recorder-design-md-an-peppy-dolphin.md`
> on 2026-08-25; that path is now a pointer stub and holds no content. Edit **this** file —
> it is live, not write-once: record every decided fork with its rationale, embed each phase's
> exhaustive subtask checklist before implementing it, and tick checkboxes as work completes.
> `Documents/` tracks the *implemented state*; this plan tracks *progress*.
...
### Decisions taken during P10-F — FYERS TBT protocol closure (2026-07-14)
15. **FYERS TBT capability is FROZEN: `tbt_budget = 15`** — 5 Market-Depth (50-level) symbols per
...
16. **Allocator consumes ONE logical `tbt_budget`; connection management (3 × 5) is hidden behind the
    ...
    Broker Capabilities → Window Manager → TBT Allocator → Allocation Policy → Subscription Manager,
    consuming `tbt_budget = 15` as one broker config.
17. **D2 reopened → HYBRID is the design** (not a fallback): near-ATM legs @50-level up to the budget, the
    rest @5-level; the Window Manager + TBT Allocator decide *which* strikes get the scarce 50-level slots
    per configured windows/policy (never hardcoded). Supersedes the P10-E "whole chain @50, no hybrid" (D2).
...
Executed live (IST Mon 2026-07-06 ~13:36–14:19, OpenAlgo + FYERS). Full detail in `plans/Plan_001_evidence/Phase9_notes.md`.
...
- [x] D3. Updated `ARCHITECTURE.md` (module map + `eod_report.py` + P8/P9/P10 built-state; storage topology
  ...
  pass + P10-A..E) — spec is the source of truth, all citing `Documents/evidence/{OPENALGO_PATCH,Phase9_notes}.md`.

### P10-E · Live validation — ✅ DONE (2026-07-07; PASS with known WARNs; full record `Documents/{LIVE_RUN,phase_10E_notes}.md`)

> ⚠️ **PARTLY SUPERSEDED (P10-F, 2026-07-14) — the "full-chain 50-level" premise below is a measurement
> artifact.** E2/E4/E9/D2 assumed all ~80 NIFTY legs streamed at 50-level. A per-second re-read of the
> **Jul-07 raw itself** shows it **never streamed >5 concurrent** NFO legs (9 distinct all session, 0
> seconds >5) — the FYERS **5-per-connection** cap was already in force. Ruled out: FYERS behavior change
> (both raws cap at 5) and OpenAlgo code change (TBT streaming files byte-identical across the Jul-10/11
...
- [x] E2. Preflight `NIFTY/NFO=50`, `SENSEX/BFO=5`; full-run raw shows **NFO `depth_levels` up to 47** across
  ...
  **→ SUPERSEDED (P10-F): the "across all 80 legs / patch works / no cap" reading is the artifact — the
  Jul-07 raw never streamed >5 concurrent NFO legs; only ≤5 lowest-strike legs streamed (at genuine 50
  levels, which is what "up to 47" reflects). The 5-per-connection cap held. See reconciliation doc.**
...
- [x] E4. **rss 52–58 MB (≪500 ✓)**; **`cycle_ms_p50` ≈ 22 ms, `max` ≈ 43–60 ms** — exceeds the old <15 ms.
  ...
  **→ SUPERSEDED (P10-F): these numbers were measured on ≤5 NFO @50-level + 120 SENSEX @5-level, NOT the
  claimed 80 × 50-level. Perf/RSS at true scale (15 × 50-level + hybrid remainder) is therefore STILL
  UNTESTED and is a real open item for the allocation-framework work.**
...
- [x] E9. `--eod-report` → **PASS 24 · WARN 2 · FAIL 0**, exit 0. WARNs = known SENSEX 5-level degrade (§9)
  ...
  `LIVE_RUN.md` §C.
  **→ SUPERSEDED (P10-F): "NIFTY depth coverage PASSES" reflects only the ≤5 legs that actually streamed,
  not the full chain — the report threshold passed on partial coverage. See reconciliation doc.**

**P10-E outcomes / open items** (detail in `plans/Plan_001_evidence/phase_10E_notes.md`):
- Patch validated end-to-end — full NIFTY chain streams true 50-level TBT; no global cap. **D2 (whole chain
  ...
  **→ SUPERSEDED (P10-F): FALSE — the full chain did NOT stream (≤5 concurrent). D2 is REOPENED: a full
  50-level chain is not achievable, so the HYBRID (near-ATM @50 + rest @5) — or a multi-connection broker
  layer to reach `tbt_budget = 15` — is now the design, no longer a mere fallback. See reconciliation doc.**
...
## Locked decisions — Generic Market-Depth Framework docs (2026-08-05)

Context: the Qwen-generated framework package in `Documents/qwen/` carried eight discrepancies against
this project's frozen facts and discipline. The docs were reconciled in place (no framework **code**
written — the "let's write the plan" gate for that effort has not been passed). Four forks were put to
the user; the answers below bind any future framework implementation phase.

- [x] **D1 — Concurrency: threads + bounded queues; every framework interface is synchronous.**
  *Rationale:* the recorder already owns four threads and three bounded queues; an event loop would be
  a second, parallel concurrency framework. The old guides' `asyncio.create_task(self.rebalance())`
  fired from a synchronous method is a silent no-op with no running loop — the window would have frozen
  at its startup strikes. Thread ownership: FEED owns the SDK client + the two-`put` tee; PROC owns
  `update_spot` / `compute_priorities` / `BudgetAllocator.allocate` / `DepthAllocator.allocate` /
  `reconcile()` (all pure, no I/O); SUBSCRIPTION owns the plan queue and **all** broker I/O; DB unchanged.
- [x] **D2 — Allocator: keep both, rename.** `BudgetAllocator` splits the broker-wide premium budget
  across underlyings; a per-underlying `DepthAllocator` then assigns premium depth to the top-N ranked
  legs. *Rationale:* the two documents described genuinely different jobs under one name; merging would
  have lost the multi-underlying split.
- [x] **D3 — Package path: `market_depth_framework/`** (not `src/market_depth/`). *Rationale:* matches
  the implementation plan; one path everywhere.
- [x] **D4 — Ops scope: rescope to single-user.** Keep circuit breaker, memory/disk monitors, and
  data-integrity validation; replace Redis / PagerDuty / HTTP health endpoints / active-active failover /
  S3 archival / user-satisfaction surveys with log-file + local-metrics-file + local-archive equivalents,
  marked **Deferred** rather than deleted. *Rationale:* one user, one host, one broker session; the
  deferred list preserves the work for a future multi-instance deployment.

Reconciled files: `planned_v1_GENERIC_FRAMEWORK_ARCHITECTURE.md`, `framework_implementation_plan.md`,
`comprehensive_implementation_guide_part1.md`, `comprehensive_implementation_guide_part2.md`,
`summary.md`. `prompt_generic_market_depth_framework.md` is the user's original requirement statement
and was left untouched by design. `summary.md` §5 carries the discrepancy→resolution table.

**Unchanged by all of the above:** the FYERS TBT protocol layer stays FROZEN — 5 Market-Depth symbols
per *connection*, 3 connections, channels are pause/resume grouping with no capacity, `tbt_budget = 15`,
string channel ids. `max_channels` never enters budget arithmetic
(`effective_budget = min(total_symbol_budget, max_connections × symbols_per_connection)`), and 15 is a
broker **capability**, not an architectural constant.

- [ ] **Deferred to the framework effort itself:** no `market_depth_framework/` code exists yet. Starting
  it requires a fresh scope-clarification → "let's write the plan" cycle; it is *not* covered by P0–P10 above.
