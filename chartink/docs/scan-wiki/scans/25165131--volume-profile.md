...
tags: ["universe:nifty-200", "indicator:volume", "timeframe:monthly", "timeframe:daily"]
...
This scan, titled "Volume Profile", appears designed to screen Indian equities in the **nifty 200** universe using **1 enabled** condition(s) combined with root join **all (AND)**.
...
Observed Chartink timeframe offsets in the tree: `0_days_ago, 1_days_ago, 1_months_ago, 20_days_ago`.
...
|---:|---|---|---|
| 1 | Disabled | daily high crossed above 1 month ago xpress indicator 1750 val( 200 ,  Both ,  20 ,  70 ,  200 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. References monthly bars / monthly offset. |
| 2 | Disabled | daily high crossed above 20 days ago xpress indicator 207 basis( 20 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. |
| 3 | Disabled | daily % change > 1 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. |
...
| 5 | Disabled | daily high crossed above daily xpress indicator 1750 val( 200 ,  Both ,  20 ,  70 ,  40 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. |
| 6 | Disabled | 1 day ago count( 5, 1 where daily high < daily xpress indicator 1750 val( 200 ,  Both ,  20 ,  70 ,  40 ) ) < 1 | Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. |
| 7 | Enabled | daily xpress indicator 3025 val( hlc3 ) crossed below daily close | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). |
