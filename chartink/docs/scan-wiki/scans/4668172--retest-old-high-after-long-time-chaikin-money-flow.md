...
This scan, titled "Retest old high after long time Chaikin Money  Flow", appears designed to screen Indian equities in the **cash** universe using **8 enabled** condition(s) combined with root join **any (OR)**.
...
| 9 | Enabled | daily cmf( 21 ) crossed above 1 day ago max( 499 ,  daily cmf( 21 ) ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. |
| 10 | Enabled | 1 day ago max( 499 ,  daily cmf( 21 ) ) < 500 days ago max( 500 ,  daily cmf( 21 ) ) | Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars. |
