...
tags: ["bias:upward-condition", "bias:downward-condition", "universe:nifty-200", "timeframe:daily", "timeframe:monthly", "timeframe:intraday-bars"]
...
This is a **multi-horizon** screen over **nifty 200** with **5** active leaf condition(s) under root join **any (OR)**.
...
- daily abs( daily accdist - 1 day ago accdist ) / daily abs( 1 day ago accdist ) * 1 > 0.4
- ( daily abs( daily accdist ) - daily abs( 1 day ago accdist ) ) / ( daily abs( 1 day ago accdist ) * 0.01 ) > 100
- [0] 60 minute accdist crossed below [-10] 60 minute min( 120 ,  [0] 60 minute accdist ) * 1
...
| 3 | 5 | Enabled | root/group[cash\|all] | daily abs( daily accdist - 1 day ago accdist ) / daily abs( 1 day ago accdist ) * 1 > 0.4 | Inequality test: left expression must be strictly greater than right. |
| 4 | 6 | Enabled | root/group[cash\|all] | ( daily abs( daily accdist ) - daily abs( 1 day ago accdist ) ) / ( daily abs( 1 day ago accdist ) * 0.01 ) > 100 | Inequality test: left expression must be strictly greater than right. |
| 5 | 8 | Enabled | root/group[cash\|all] | [0] 60 minute accdist crossed below [-10] 60 minute min( 120 ,  [0] 60 minute accdist ) * 1 | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
