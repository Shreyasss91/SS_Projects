...
source_url: https://chartink.com/screener/copy-3-week-tight-close-omkarbanne-1596
market: Indian equities
horizon: Multi-horizon
classification: ["Moving average", "Volume/delivery", "Breakout", "Multi-factor"]
tags: ["bias:upward-condition", "universe:cash", "indicator:volume", "indicator:ema", "indicator:sma", "timeframe:daily", "timeframe:monthly", "timeframe:weekly"]
...
# Copy - 3 Week Tight Close - OmkarBanne

## Source

- Chartink URL: https://chartink.com/screener/copy-3-week-tight-close-omkarbanne-1596
- Scan ID: `14001530`
- Slug: `copy-3-week-tight-close-omkarbanne-1596`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Multi-horizon
- Created at (Chartink): 2023-11-30T02:44:30.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/14001530.json](../source-snapshots/14001530.json)
- Text snapshot: [source-snapshots/14001530.txt](../source-snapshots/14001530.txt)

## What this scan is for

This is a **multi-horizon** screen over **cash** with **5** active leaf condition(s) under root join **all (AND)**.
...
Scan id: 14001530
Slug: copy-3-week-tight-close-omkarbanne-1596
Source URL: https://chartink.com/screener/copy-3-week-tight-close-omkarbanne-1596
Root universe/segment: cash
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2023-11-30T02:44:30.000000Z
...
3. [Enabled] daily sma( close ,  50 ) >= 10000
    group_path: root/group[cash|all]
4. [Enabled] daily close > daily ema( close ,  50 )
    group_path: root/group[cash|all]
5. [Enabled] daily abs( ( weekly max( 3 ,  weekly close ) / weekly min( 3 ,  weekly close ) - 1 ) * 100 ) <= 2
    group_path: root/group[cash|all]
6. [Disabled] daily abs( ( weekly max( 3 ,  weekly high ) / weekly min( 3 ,  weekly low ) - 1 ) * 100 ) <= 10
    group_path: root/group[cash|all]
...
- **#2** `daily high > 100` — Inequality test: left expression must be strictly greater than right.
- **#3** `daily sma( close ,  50 ) >= 10000` — Inequality test: left expression must be greater than or equal to right. SMA is the arithmetic mean of the chosen field over N bars.
...
- **#5** `daily abs( ( weekly max( 3 ,  weekly close ) / weekly min( 3 ,  weekly close ) - 1 ) * 100 ) <= 2` — Inequality test: left expression must be less than or equal to right. max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. References weekly bars / weekly offset.
- **#7** `( 3 weeks ago max( 12 ,  weekly close ) / 3 weeks ago min( 12 ,  weekly close ) - 1 ) * 100 >= 30` — Inequality test: left expression must be greater than or equal to right. max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. References weekly bars / weekly offset.
...
### Disabled #6
- **Condition (verbatim):** `daily abs( ( weekly max( 3 ,  weekly high ) / weekly min( 3 ,  weekly low ) - 1 ) * 100 ) <= 10`
- **Meaning:** Inequality test: left expression must be less than or equal to right. Currently disabled in source — not applied when the scan runs. max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. References weekly bars / weekly offset.
...
- Timeframe tokens: `0_days_ago`, `0_months_ago`, `0_weeks_ago`, `3_weeks_ago`
