...
- Chartink URL: https://chartink.com/screener/trade-book-25
- Scan ID: `24462563`
- Slug: `trade-book-25`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2025-11-12T15:36:55.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/24462563.json](../source-snapshots/24462563.json)
- Text snapshot: [source-snapshots/24462563.txt](../source-snapshots/24462563.txt)
...
Scan id: 24462563
Slug: trade-book-25
Source URL: https://chartink.com/screener/trade-book-25
...
- **#3** `[0] 15 minute sum( close ,  25 ) crossed above [-25] 15 minute sum( close ,  25 ) * 10` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Disabled #1
- **Condition (verbatim):** `[0] 15 minute sum( close ,  25 ) crossed above [0] 15 minute sma( close ,  150 )`
- **Meaning:** Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.

### Disabled #2
- **Condition (verbatim):** `[0] 15 minute sum( close ,  25 ) crossed above [-25] 15 minute sma( close ,  1 ) * 5`
- **Meaning:** Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.
...
- `cancelled sell orders quantity` — appears 6 time(s) in the expression tree
- `cancelled buy orders quantity: ` — appears 6 time(s) in the expression tree
