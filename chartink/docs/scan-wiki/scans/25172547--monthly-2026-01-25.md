...
tags: ["universe:nifty-200", "indicator:volume", "indicator:sma", "timeframe:monthly", "timeframe:daily"]
...
This scan, titled "Monthly 2026-01-25", appears designed to screen Indian equities in the **nifty 200** universe using **2 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Moving average, Volume/delivery**. Likely horizon label from name/timeframes: **Positional**.
...
|---:|---|---|---|
| 1 | Disabled | monthly open = monthly low | Equality test between left and right expressions. Currently disabled in source — not applied when the scan runs. References monthly bars / monthly offset. |
