...
This scan, titled "Ichimoku Std Deviation", appears designed to screen Indian equities in the **nifty 200** universe using **1 enabled** condition(s) combined with root join **all (AND)**.
...
|---:|---|---|---|
| 1 | Disabled | daily close crossed below daily stddva( close ,  1200 ) | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). Currently disabled in source — not applied when the scan runs. |
| 2 | Disabled | [0] 60 minute close crossed above [0] 60 minute stddva( close ,  1200 ) + [0] 60 minute ichimoku base line( 233 ,  466 ,  932 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. Ichimoku components (conversion/base/spans) describe equilibrium and cloud structure. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 3 | Enabled | [0] 60 minute high crossed above [0] 60 minute ichimoku base line( 233 ,  466 ,  932 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Ichimoku components (conversion/base/spans) describe equilibrium and cloud structure. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
