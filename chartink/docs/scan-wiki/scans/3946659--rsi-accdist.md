...
- Chartink URL: https://chartink.com/screener/natural-lan-2
- Scan ID: `3946659`
- Slug: `natural-lan-2`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Swing
- Created at (Chartink): 2021-02-13T13:55:02.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/3946659.json](../source-snapshots/3946659.json)
- Text snapshot: [source-snapshots/3946659.txt](../source-snapshots/3946659.txt)
...
Scan id: 3946659
Slug: natural-lan-2
Source URL: https://chartink.com/screener/natural-lan-2
...
3. [Disabled] daily MY_RSI crossed above 20
4. [Enabled] daily rsi( 14 ) > 90
5. [Enabled] daily rsi( 50 ) > 90
6. [Enabled] daily rsi( 100 ) > 90
7. [Disabled] [GROUP segment=futures join=any combination=passes measurevalue=default]  (path: root/group[futures|any])
8. [Enabled] daily ln_x crossed above 1 day ago min( 365 ,  daily ln_x )
...
- **#4** `daily rsi( 14 ) > 90` — Inequality test: left expression must be strictly greater than right. RSI is a momentum oscillator from average gains/losses over its period.
- **#5** `daily rsi( 50 ) > 90` — Inequality test: left expression must be strictly greater than right. RSI is a momentum oscillator from average gains/losses over its period.
- **#6** `daily rsi( 100 ) > 90` — Inequality test: left expression must be strictly greater than right. RSI is a momentum oscillator from average gains/losses over its period.
- **#8** `daily ln_x crossed above 1 day ago min( 365 ,  daily ln_x )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). min(N, series) is the lowest value of series over N bars.
...
### Disabled #2
- **Condition (verbatim):** `daily MY_RSI crossed above 1 day ago min( 365 ,  daily MY_RSI )`
- **Meaning:** Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. RSI is a momentum oscillator from average gains/losses over its period. min(N, series) is the lowest value of series over N bars.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.

### Disabled #3
- **Condition (verbatim):** `daily MY_RSI crossed above 20`
- **Meaning:** Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. RSI is a momentum oscillator from average gains/losses over its period.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.
...
### Measures observed
- `custom_indicator_13107` — appears 3 time(s) in the expression tree
