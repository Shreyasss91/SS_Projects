...
# MORNING HURRY_EVENING HURRY

## Source

- Chartink URL: https://chartink.com/screener/morning-hurry
- Scan ID: `22156174`
- Slug: `morning-hurry`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2025-05-30T06:14:54.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/22156174.json](../source-snapshots/22156174.json)
- Text snapshot: [source-snapshots/22156174.txt](../source-snapshots/22156174.txt)
...
Scan id: 22156174
Slug: morning-hurry
Source URL: https://chartink.com/screener/morning-hurry
...
3. [Disabled] [1] 60 minute volume / daily volume > 0.6
    group_path: root/group[cash|all]
4. [Disabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
5. [Disabled] [1] 60 minute buyer initiated trades quantity / [1] 60 minute seller initiated trades quantity > 4
    group_path: root/group[cash|all]
6. [Enabled] [6] 60 minute buyer initiated trades quantity / [6] 60 minute seller initiated trades quantity > 4
    group_path: root/group[cash|all]
7. [Enabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
8. [Enabled] ( [6] 60 minute volume + [7] 60 minute volume ) / daily volume > 0.75
    group_path: root/group[cash|all]
...
- **#2** `[1] 60 minute volume / daily volume > 0.75` — Inequality test: left expression must be strictly greater than right. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#6** `[6] 60 minute buyer initiated trades quantity / [6] 60 minute seller initiated trades quantity > 4` — Inequality test: left expression must be strictly greater than right. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#8** `( [6] 60 minute volume + [7] 60 minute volume ) / daily volume > 0.75` — Inequality test: left expression must be strictly greater than right. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Disabled #3
- **Condition (verbatim):** `[1] 60 minute volume / daily volume > 0.6`
...
### Disabled #5
- **Condition (verbatim):** `[1] 60 minute buyer initiated trades quantity / [1] 60 minute seller initiated trades quantity > 4`
...
### Disabled #9
- **Condition (verbatim):** `( [6] 60 minute volume + [7] 60 minute volume ) / daily volume > 0.6`
- **Meaning:** Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.
...
- `buyer initiated trades quantity` — appears 2 time(s) in the expression tree
- `seller initiated trades quantity` — appears 2 time(s) in the expression tree
