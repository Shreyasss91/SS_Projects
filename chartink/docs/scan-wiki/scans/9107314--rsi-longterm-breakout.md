...
This scan, titled "rsi longterm breakout", appears designed to screen Indian equities in the **nifty 200** universe using **13 enabled** condition(s) combined with root join **any (OR)**.
...
| 3 | Enabled | [-50] 5 minute sma( close ,  250 ) < 50 | Inequality test: left expression must be strictly less than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
