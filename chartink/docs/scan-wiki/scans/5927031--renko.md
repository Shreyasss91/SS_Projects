...
- [0] 60 minute low crossed below [-1] 60 minute min( 1440 ,  [0] 60 minute low ) * 1.05
...
| 2 | 2 | Enabled | root | daily earning per share > daily p earning per share | Inequality test: left expression must be strictly greater than right. |
| 3 | 4 | Enabled | root/group[cash\|all] | [0] 60 minute low crossed below [-1] 60 minute min( 1440 ,  [0] 60 minute low ) * 1.05 | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 4 | 5 | Disabled | root/group[cash\|all] | 20 days ago min( 480 ,  daily low ) < 500 days ago min( 500 ,  daily low ) | Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. min(N, series) is the lowest value of series over N bars. |
