...
This scan, titled "Big GAPUP as Base__Plus Shakeout___BULLISH", appears designed to screen Indian equities in the **nifty 200** universe using **2 enabled** condition(s) combined with root join **all (AND)**.
...
|---:|---|---|---|
| 1 | Disabled | daily count( 120, 1 where daily open > 1 day ago close * 1.08 ) >= 1 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. |
| 2 | Enabled | daily low < daily close * 0.92 | Inequality test: left expression must be strictly less than right. |
| 3 | Enabled | daily close > 30 | Inequality test: left expression must be strictly greater than right. |
