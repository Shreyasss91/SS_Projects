...
horizon: Swing
classification: ["Moving average", "Price action", "Trend following", "Volatility", "Volume/delivery", "Multi-factor"]
...
This scan, titled "trend change to bull", appears designed to screen Indian equities in the **cash** universe using **5 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Moving average, Price action, Trend following, Volatility**. Likely horizon label from name/timeframes: **Swing**.
...
| 5 | Enabled | daily % change > 8 | Inequality test: left expression must be strictly greater than right. |
| 6 | Enabled | daily volume > 1 day ago sma( close ,  7 ) * 15 | Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity. |
...
- **Horizon:** Swing
- **Methods:** Moving average, Price action, Trend following, Volatility, Volume/delivery, Multi-factor
- **Tags:** universe:cash, indicator:volume, indicator:sma, timeframe:daily
- **Root universe:** cash
- **Root join:** all
