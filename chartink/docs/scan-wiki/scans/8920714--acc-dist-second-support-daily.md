---
scan_id: 8920714
scan_name: acc dist second support daily
source_url: https://chartink.com/screener/acc-dist-second-support-daily
market: Indian equities
horizon: Multi-horizon
classification: ["Support/resistance", "Volume/delivery", "Momentum", "Multi-factor"]
tags: ["universe:nifty-50", "timeframe:intraday-bars", "timeframe:weekly", "timeframe:daily"]
...
# acc dist second support daily

## Source

- Chartink URL: https://chartink.com/screener/acc-dist-second-support-daily
- Scan ID: `8920714`
- Slug: `acc-dist-second-support-daily`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Multi-horizon
- Created at (Chartink): 2022-07-01T15:24:17.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/8920714.json](../source-snapshots/8920714.json)
- Text snapshot: [source-snapshots/8920714.txt](../source-snapshots/8920714.txt)

## What this scan is for

This scan, titled "acc dist second support daily", appears designed to screen Indian equities in the **nifty 500** universe using **4 enabled** condition(s) combined with root join **any (OR)**.

Dominant method tag(s) inferred from conditions: **Support/resistance, Volume/delivery, Momentum, Multi-factor**. Likely horizon label from name/timeframes: **Multi-horizon**.
...
```text
Scan name: acc dist second support daily
Scan id: 8920714
Slug: acc-dist-second-support-daily
Source URL: https://chartink.com/screener/acc-dist-second-support-daily
Root universe/segment: nifty 500
Root join: any (OR)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2022-07-01T15:24:17.000000Z

=== Condition tree (from atlas_json; includes Enabled and Disabled) ===

1. [Enabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
2. [Enabled] 1 day ago accdist - ( 1 day ago max( 21 ,  daily accdist ) - 1 day ago min( 21 ,  daily accdist ) ) crossed below daily accdist
    group_path: root/group[cash|all]
3. [Enabled] daily low < 1 day ago min( 21 ,  daily low )
    group_path: root/group[cash|all]
...
( nifty 500 ( ( cash ( 1 day ago accdist  - ( 1 day ago max( 21 , latest accdist  ) - 1 day ago min( 21 , latest accdist  ) ) < latest accdist  and 2 day ago  accdist  - ( 2 day ago  max( 21 , latest accdist  )- 2 day ago  min( 21 , latest accdist  )) >= 1 day ago  accdist  and latest low < 1 day ago min( 21 , latest low ) ) ) ) )
```
...
| 2 | Enabled | 1 day ago accdist - ( 1 day ago max( 21 ,  daily accdist ) - 1 day ago min( 21 ,  daily accdist ) ) crossed below daily accdist | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. |
| 3 | Enabled | daily low < 1 day ago min( 21 ,  daily low ) | Inequality test: left expression must be strictly less than right. min(N, series) is the lowest value of series over N bars. |
...
| 5 | Enabled | 1 week ago accdist - ( 1 week ago max( 21 ,  1 week ago accdist ) - 1 week ago min( 21 ,  1 week ago accdist ) ) crossed above weekly accdist | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. References weekly bars / weekly offset. |
| 6 | Enabled | daily high > 1 day ago max( 21 ,  daily high ) | Inequality test: left expression must be strictly greater than right. max(N, series) is the highest value of series over N bars. |
| 7 | Disabled | [GROUP segment=nifty 500 join=all combination=passes measurevalue=default] | Nested group over segment **nifty 500** with join **all** (combination=passes). Group status=Disabled. |
| 8 | Disabled | [-1] 60 minute accdist + ( [-1] 60 minute max( 28 ,  [0] 60 minute accdist ) - [-1] 60 minute min( 28 ,  [0] 60 minute accdist ) ) crossed above [0] 60 minute accdist | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 9 | Disabled | [-1] 120 minute accdist + ( [-1] 120 minute max( 21 ,  [0] 120 minute accdist ) - [-1] 120 minute min( 21 ,  [0] 120 minute accdist ) ) crossed above [0] 120 minute accdist | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **#2** `1 day ago accdist - ( 1 day ago max( 21 ,  daily accdist ) - 1 day ago min( 21 ,  daily accdist ) ) crossed below daily accdist` — Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars.
- **#3** `daily low < 1 day ago min( 21 ,  daily low )` — Inequality test: left expression must be strictly less than right. min(N, series) is the lowest value of series over N bars.
- **#5** `1 week ago accdist - ( 1 week ago max( 21 ,  1 week ago accdist ) - 1 week ago min( 21 ,  1 week ago accdist ) ) crossed above weekly accdist` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. References weekly bars / weekly offset.
