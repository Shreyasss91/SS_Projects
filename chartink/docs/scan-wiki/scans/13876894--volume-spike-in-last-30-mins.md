...
horizon: Intraday
classification: ["Volume/delivery", "Moving average", "Price action", "Volatility", "Multi-factor"]
...
- Chartink URL: https://chartink.com/screener/volume-spike-in-last-30-mins
- Scan ID: `13876894`
- Slug: `volume-spike-in-last-30-mins`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2023-11-20T02:20:11.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/13876894.json](../source-snapshots/13876894.json)
- Text snapshot: [source-snapshots/13876894.txt](../source-snapshots/13876894.txt)

## What this scan is for

This scan, titled "volume spike in last 30 mins", appears designed to screen Indian equities in the **cash** universe using **4 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Volume/delivery, Moving average, Price action, Volatility**. Likely horizon label from name/timeframes: **Intraday**.
...
```text
Scan name: volume spike in last 30 mins
Scan id: 13876894
Slug: volume-spike-in-last-30-mins
Source URL: https://chartink.com/screener/volume-spike-in-last-30-mins
Root universe/segment: cash
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2023-11-20T02:20:11.000000Z
...
4. [Enabled] daily close > 50
    group_path: root/group[cash|all]
5. [Enabled] [13] 30 minute sum( close ,  2 ) / [13] 30 minute sma( close ,  7 ) > 3
6. [Enabled] daily % change < 1

=== Chartink atlas_query (compiled/active form; typically omits disabled filters) ===

( cash ( ( cash ( 1 day ago close * 1 day ago volume > 100000000 and latest close > 50 ) ) and [=13] 30 minute sum( [0] 30 minute volume , 2 ) / [=13] 30 minute sma( [=-1] 30 minute sum( [0] 30 minute volume , 2 ) , 7 ) > 3 and latest "close - 1 candle ago close / 1 candle ago close * 100" < 1 ) )
```
...
| 5 | Enabled | [13] 30 minute sum( close ,  2 ) / [13] 30 minute sma( close ,  7 ) > 3 | Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **#5** `[13] 30 minute sum( close ,  2 ) / [13] 30 minute sma( close ,  7 ) > 3` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
- **Horizon:** Intraday
- **Methods:** Volume/delivery, Moving average, Price action, Volatility, Multi-factor
- **Tags:** universe:cash, indicator:volume, indicator:sma, timeframe:intraday-bars, timeframe:daily
- **Root universe:** cash
- **Root join:** all
