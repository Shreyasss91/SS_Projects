...
# Price sum (longterm...for bulk money investment)

## Source

- Chartink URL: https://chartink.com/screener/price-sum-longterm-for-bulk-money-investment
- Scan ID: `9622291`
- Slug: `price-sum-longterm-for-bulk-money-investment`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Swing
- Created at (Chartink): 2022-09-12T02:03:23.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/9622291.json](../source-snapshots/9622291.json)
- Text snapshot: [source-snapshots/9622291.txt](../source-snapshots/9622291.txt)

## What this scan is for

This scan, titled "Price sum (longterm...for bulk money investment)", appears designed to screen Indian equities in the **cash** universe using **2 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Volume/delivery, Momentum**. Likely horizon label from name/timeframes: **Swing**.

Observed Chartink timeframe offsets in the tree: `0_days_ago, 14_days_ago, 1_days_ago`.
...
```text
Scan name: Price sum (longterm...for bulk money investment)
Scan id: 9622291
Slug: price-sum-longterm-for-bulk-money-investment
Source URL: https://chartink.com/screener/price-sum-longterm-for-bulk-money-investment
Root universe/segment: cash
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2022-09-12T02:03:23.000000Z
...
4. [Enabled] daily sum( close ,  80 ) crossed below 14 days ago min( 800 ,  daily sum( close ,  80 ) )
    group_path: root/group[cash|all]

=== Chartink atlas_query (compiled/active form; typically omits disabled filters) ===

( cash ( ( cash ( 1 day ago close * 1 day ago volume > 100000000 ) ) and( cash ( latest sum( latest close - latest open , 80 ) < 14 days ago min( 800 , latest sum( latest close - latest open , 80 ) ) and 1 day ago  sum( latest close - 1 day ago  open , 80 ) >= 15 days ago  min( 800 , latest sum( latest close - latest open , 80 ) ) ) ) ) )
```
...
| 4 | Enabled | daily sum( close ,  80 ) crossed below 14 days ago min( 800 ,  daily sum( close ,  80 ) ) | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. |
...
- **#4** `daily sum( close ,  80 ) crossed below 14 days ago min( 800 ,  daily sum( close ,  80 ) )` — Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars.
...
- Timeframe tokens: `0_days_ago`, `14_days_ago`, `1_days_ago`
