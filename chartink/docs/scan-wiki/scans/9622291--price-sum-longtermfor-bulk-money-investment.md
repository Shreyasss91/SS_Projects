...
| 2 | 4 | Enabled | root/group[cash\|all] | daily sum( close ,  80 ) crossed below 14 days ago min( 800 ,  daily sum( close ,  80 ) ) | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. |
