...
tags: ["bias:upward-condition", "bias:downward-condition", "universe:nifty-200", "indicator:vwap", "indicator:volume", "indicator:sma", "timeframe:daily", "timeframe:intraday-bars"]
...
- Chartink URL: https://chartink.com/screener/test-2019-11-29-7
- Scan ID: `1464592`
- Slug: `test-2019-11-29-7`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2019-11-29T12:59:03.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/1464592.json](../source-snapshots/1464592.json)
- Text snapshot: [source-snapshots/1464592.txt](../source-snapshots/1464592.txt)
...
Scan id: 1464592
Slug: test-2019-11-29-7
Source URL: https://chartink.com/screener/test-2019-11-29-7
...
3. [Disabled] 2 days ago close < daily sma( vwap,200 )
4. [Disabled] 1 day ago close > daily sma( vwap,200 )
5. [Disabled] daily close > daily sma( vwap,200 )
6. [Enabled] [-2] 30 minute close < [0] 30 minute sma( close ,  200 )
...
- **#6** `[-2] 30 minute close < [0] 30 minute sma( close ,  200 )` — Inequality test: left expression must be strictly less than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#7** `[-1] 30 minute close > [0] 30 minute sma( close ,  200 )` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#8** `[0] 30 minute close > [0] 30 minute sma( close ,  200 )` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Disabled #3
- **Condition (verbatim):** `2 days ago close < daily sma( vwap,200 )`
- **Meaning:** Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. VWAP is volume-weighted average price for the session/period context Chartink supplies.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.

### Disabled #4
- **Condition (verbatim):** `1 day ago close > daily sma( vwap,200 )`
- **Meaning:** Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. VWAP is volume-weighted average price for the session/period context Chartink supplies.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.

### Disabled #5
- **Condition (verbatim):** `daily close > daily sma( vwap,200 )`
- **Meaning:** Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. VWAP is volume-weighted average price for the session/period context Chartink supplies.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.
...
- `vwap` — appears 3 time(s) in the expression tree
...
- Timeframe tokens: `0_days_ago`, `1_days_ago`, `2_days_ago`, `30_minute`
