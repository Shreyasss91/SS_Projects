...
This scan, titled "retest low", appears designed to screen Indian equities in the **cash** universe using **3 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Mean reversion, Volume/delivery, Momentum, Multi-factor**. Likely horizon label from name/timeframes: **Swing**.
...
| 3 | Enabled | daily low crossed below 1 day ago min( 365 ,  daily low ) | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. |
