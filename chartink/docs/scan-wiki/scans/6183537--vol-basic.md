...
This scan, titled "vol basic", appears designed to screen Indian equities in the **cash** universe using **3 enabled** condition(s) combined with root join **all (AND)**.
...
Observed Chartink timeframe offsets in the tree: `0_days_ago, 1_days_ago, 30_minute, 3_days_ago`.
...
| 2 | Enabled | daily count( 20, 1 where daily high = daily low ) < 1 | Inequality test: left expression must be strictly less than right. |
| 3 | Disabled | daily volume > 3 days ago sma( close ,  7 ) * 10 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity. |
| 4 | Enabled | [0] 30 minute volume > [-1] 30 minute sma( close ,  7 ) * 10 | Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
