...
tags: ["bias:upward-condition", "bias:downward-condition", "universe:nifty-200", "indicator:atr", "indicator:ema", "timeframe:daily", "timeframe:intraday-bars"]
...
- Chartink URL: https://chartink.com/screener/fresh-impulse-alerter
- Scan ID: `24504096`
- Slug: `fresh-impulse-alerter`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2025-11-16T15:35:40.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/24504096.json](../source-snapshots/24504096.json)
- Text snapshot: [source-snapshots/24504096.txt](../source-snapshots/24504096.txt)

## What this scan is for

This is a **intraday** screen over **nifty 200** with **5** active leaf condition(s) under root join **any (OR)**.
...
Scan id: 24504096
Slug: fresh-impulse-alerter
Source URL: https://chartink.com/screener/fresh-impulse-alerter
...
3. [Enabled] [0] 60 minute avg true range( 14 ) crossed above [-1] 60 minute max( 120 ,  [0] 60 minute avg true range( 14 ) )
    group_path: root/group[cash|all]
4. [Disabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
5. [Enabled] [0] 60 minute count( 75, 1 where [0] 60 minute avg true range( 14 ) < [0] 60 minute ema( close ,  233 ) ) > 70
    group_path: root/group[cash|all]
6. [Enabled] [0] 60 minute avg true range( 14 ) crossed above [0] 60 minute ema( close ,  233 )
    group_path: root/group[cash|all]
7. [Disabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
8. [Enabled] [-1] 60 minute avg true range( 14 ) crossed above [-1] 60 minute ema( close ,  233 )
    group_path: root/group[cash|all]
...
- **#3** `[0] 60 minute avg true range( 14 ) crossed above [-1] 60 minute max( 120 ,  [0] 60 minute avg true range( 14 ) )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). ATR measures smoothed true range (volatility), not direction. max(N, series) is the highest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#5** `[0] 60 minute count( 75, 1 where [0] 60 minute avg true range( 14 ) < [0] 60 minute ema( close ,  233 ) ) > 70` — Inequality test: left expression must be strictly greater than right. EMA is an exponentially weighted moving average of the chosen field. ATR measures smoothed true range (volatility), not direction. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#6** `[0] 60 minute avg true range( 14 ) crossed above [0] 60 minute ema( close ,  233 )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). EMA is an exponentially weighted moving average of the chosen field. ATR measures smoothed true range (volatility), not direction. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#8** `[-1] 60 minute avg true range( 14 ) crossed above [-1] 60 minute ema( close ,  233 )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). EMA is an exponentially weighted moving average of the chosen field. ATR measures smoothed true range (volatility), not direction. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#9** `[0] 60 minute avg true range( 14 ) > [-1] 60 minute ema( close ,  233 ) * 1.15` — Inequality test: left expression must be strictly greater than right. EMA is an exponentially weighted moving average of the chosen field. ATR measures smoothed true range (volatility), not direction. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Disabled #2
- **Condition (verbatim):** `[0] 60 minute avg true range( 14 ) crossed above [-1] 60 minute max( 75 ,  [0] 60 minute avg true range( 14 ) )`
...
### Measures observed
- `avg true range` — appears 12 time(s) in the expression tree
