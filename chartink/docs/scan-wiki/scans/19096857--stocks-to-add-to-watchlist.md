...
- Chartink URL: https://chartink.com/screener/stocks-to-add-to-watchlist
- Scan ID: `19096857`
- Slug: `stocks-to-add-to-watchlist`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Positional
- Created at (Chartink): 2024-10-20T05:58:01.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/19096857.json](../source-snapshots/19096857.json)
- Text snapshot: [source-snapshots/19096857.txt](../source-snapshots/19096857.txt)

## What this scan is for

This is a **positional** screen over **nifty 200** with **7** active leaf condition(s) under root join **all (AND)**.
...
Scan id: 19096857
Slug: stocks-to-add-to-watchlist
Source URL: https://chartink.com/screener/stocks-to-add-to-watchlist
...
3. [Enabled] daily abs( 1 - ( daily close / 1 month ago high ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
4. [Enabled] daily abs( 1 - ( daily high / 1 month ago high ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
5. [Enabled] daily abs( 1 - ( daily low / 1 month ago high ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
6. [Enabled] [GROUP segment=cash join=any combination=passes measurevalue=default]  (path: root/group[cash|any]/group[cash|any])
7. [Enabled] daily abs( 1 - ( daily close / 1 month ago low ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
8. [Enabled] daily abs( 1 - ( daily low / 1 month ago low ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
9. [Enabled] daily abs( 1 - ( daily high / 1 month ago low ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
10. [Enabled] [GROUP segment=cash join=any combination=passes measurevalue=default]  (path: root/group[cash|any])
11. [Enabled] [GROUP segment=cash join=any combination=passes measurevalue=default]  (path: root/group[cash|any]/group[cash|any])
12. [Disabled] daily abs( 1 - ( daily close / ( 1 month ago low - ( 0.618 * 1 month ago true range( 1 ) ) ) ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
13. [Disabled] daily abs( 1 - ( daily close / ( 1 month ago high + ( 0.618 * 1 month ago true range( 1 ) ) ) ) ) < 0.01
    group_path: root/group[cash|any]/group[cash|any]
...
- **#3** `daily abs( 1 - ( daily close / 1 month ago high ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#4** `daily abs( 1 - ( daily high / 1 month ago high ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#5** `daily abs( 1 - ( daily low / 1 month ago high ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#7** `daily abs( 1 - ( daily close / 1 month ago low ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#8** `daily abs( 1 - ( daily low / 1 month ago low ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#9** `daily abs( 1 - ( daily high / 1 month ago low ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
- **#14** `daily abs( 1 - ( daily close / ( 1 month ago high + ( 0.618 * 1 quarters ago true range( 1 ) ) ) ) ) < 0.01` — Inequality test: left expression must be strictly less than right. References monthly bars / monthly offset.
...
### Disabled #12
- **Condition (verbatim):** `daily abs( 1 - ( daily close / ( 1 month ago low - ( 0.618 * 1 month ago true range( 1 ) ) ) ) ) < 0.01`
- **Meaning:** Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. References monthly bars / monthly offset.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.

### Disabled #13
- **Condition (verbatim):** `daily abs( 1 - ( daily close / ( 1 month ago high + ( 0.618 * 1 month ago true range( 1 ) ) ) ) ) < 0.01`
- **Meaning:** Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. References monthly bars / monthly offset.
- **If enabled:** would add this constraint to the active boolean tree (subject to its group join), likely changing candidate count and timing.
- **Trade-offs:** enabling usually increases selectivity and may remove early or noisy matches; keeping it disabled preserves a wider (or differently timed) set of results.
...
### Measures observed
- `abs` — appears 9 time(s) in the expression tree
...
- `true range` — appears 3 time(s) in the expression tree
...
- Timeframe tokens: `0_days_ago`, `1_months_ago`, `1_quarters_ago`
