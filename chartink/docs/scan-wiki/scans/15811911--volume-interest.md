...
source_url: https://chartink.com/screener/volume-interest-2
...
- Chartink URL: https://chartink.com/screener/volume-interest-2
- Scan ID: `15811911`
- Slug: `volume-interest-2`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2024-04-09T15:53:48.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/15811911.json](../source-snapshots/15811911.json)
- Text snapshot: [source-snapshots/15811911.txt](../source-snapshots/15811911.txt)

## What this scan is for

This scan, titled "volume interest", appears designed to screen Indian equities in the **futures** universe using **1 enabled** condition(s) combined with root join **all (AND)**.
...
Author description (source metadata): For shortlisting stocks into Radar
...
Scan id: 15811911
Slug: volume-interest-2
Source URL: https://chartink.com/screener/volume-interest-2
Root universe/segment: futures
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2024-04-09T15:53:48.000000Z

=== Condition tree (from atlas_json; includes Enabled and Disabled) ===

1. [Disabled] [0] 1 minute sma( close ,  3 ) > [-5] 1 minute sma( close ,  60 ) * 100
2. [Enabled] [0] 5 minute volume > [-2] 5 minute sma( close ,  12 ) * 20

=== Chartink atlas_query (compiled/active form; typically omits disabled filters) ===

( futures ( [0] 5 minute volume > [-2] 5 minute sma( [0] 5 minute volume , 12 ) * 20 ) )
```
...
|---:|---|---|---|
| 1 | Disabled | [0] 1 minute sma( close ,  3 ) > [-5] 1 minute sma( close ,  60 ) * 100 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. SMA is the arithmetic mean of the chosen field over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 2 | Enabled | [0] 5 minute volume > [-2] 5 minute sma( close ,  12 ) * 20 | Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **#2** `[0] 5 minute volume > [-2] 5 minute sma( close ,  12 ) * 20` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Disabled #1
- **Condition (verbatim):** `[0] 1 minute sma( close ,  3 ) > [-5] 1 minute sma( close ,  60 ) * 100`
...
- Timeframe tokens: `0_days_ago`, `1_minute`, `5_minute`
