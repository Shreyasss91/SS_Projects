...
This scan, titled "STRONG PRICE REJECTIONS_index_BANKNIFTY", appears designed to screen Indian equities in the **BANKNIFTY_INDEX** universe using **2 enabled** condition(s) combined with root join **any (OR)**.
...
| 2 | Enabled | daily high - daily greatest > 250 | Inequality test: left expression must be strictly greater than right. |
...
| 5 | Enabled | daily high - daily greatest = daily max( 30 ,  daily high - daily greatest ) | Equality test between left and right expressions. max(N, series) is the highest value of series over N bars. |
