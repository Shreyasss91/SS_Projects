---
scan_id: 4915531
scan_name: "Retest old high after long time ... 90% range"
source_url: https://chartink.com/screener/retest-old-high-after-long-time-90-range
market: Indian equities
horizon: Intraday
classification: ["Mean reversion", "Volatility", "Breakout", "Volume/delivery", "Momentum", "Multi-factor"]
...
# Retest old high after long time ... 90% range

## Source

- Chartink URL: https://chartink.com/screener/retest-old-high-after-long-time-90-range
- Scan ID: `4915531`
- Slug: `retest-old-high-after-long-time-90-range`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2021-06-11T15:06:46.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/4915531.json](../source-snapshots/4915531.json)
- Text snapshot: [source-snapshots/4915531.txt](../source-snapshots/4915531.txt)

## What this scan is for

This scan, titled "Retest old high after long time ... 90% range", appears designed to screen Indian equities in the **futures** universe using **12 enabled** condition(s) combined with root join **any (OR)**.

Dominant method tag(s) inferred from conditions: **Mean reversion, Volatility, Breakout, Volume/delivery**. Likely horizon label from name/timeframes: **Intraday**.

Observed Chartink timeframe offsets in the tree: `0_days_ago, 15_minute, 1_days_ago, 20_days_ago, 21_days_ago, 500_days_ago`.
...
```text
Scan name: Retest old high after long time ... 90% range
Scan id: 4915531
Slug: retest-old-high-after-long-time-90-range
Source URL: https://chartink.com/screener/retest-old-high-after-long-time-90-range
Root universe/segment: futures
Root join: any (OR)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2021-06-11T15:06:46.000000Z
...
8. [Enabled] daily count( 200, 1 where ( daily high / daily low ) = 1 ) < 1
    group_path: root/group[cash|all]
9. [Enabled] daily high crossed above 1 day ago min( 499 ,  daily low ) + ( 1 day ago max( 499 ,  daily high ) - 1 day ago min( 499 ,  daily low ) ) * 0.9
    group_path: root/group[cash|all]
...
13. [Enabled] daily count( 200, 1 where ( daily high / daily low ) = 1 ) < 1
    group_path: root/group[cash|all]
14. [Enabled] [0] 15 minute high crossed above [-1] 15 minute min( 499 ,  [0] 15 minute low ) + ( ( [-1] 15 minute max( 499 ,  [0] 15 minute high ) - [-1] 15 minute min( 499 ,  [0] 15 minute low ) ) * 0.9 )
    group_path: root/group[cash|all]
15. [Enabled] [-1] 15 minute max( 499 ,  [0] 15 minute high ) < [-500] 15 minute max( 500 ,  [0] 15 minute high )
    group_path: root/group[cash|all]

=== Chartink atlas_query (compiled/active form; typically omits disabled filters) ===

( futures ( ( cash ( 1 day ago close * 1 day ago volume > 100000000 and latest count( 200, 1 where( latest high / latest low ) = 1 ) < 1 and [0] 15 minute high > [-1] 15 minute min( 499 , [0] 15 minute low ) + ( ( [-1] 15 minute max( 499 , [0] 15 minute high ) - [-1] 15 minute min( 499 , [0] 15 minute low ) ) * 0.9 ) and [ -1 ] 15 minute high <= [ -2 ] 15 minute min( 499 , [0] 15 minute low )+ ( ( [ -2 ] 15 minute max( 499 , [0] 15 minute high )- [ -2 ] 15 minute min( 499 , [0] 15 minute low )) * 0.9 ) and [-1] 15 minute max( 499 , [0] 15 minute high ) < [-500] 15 minute max( 500 , [0] 15 minute high ) ) ) ) )
```
...
| 14 | Enabled | [0] 15 minute high crossed above [-1] 15 minute min( 499 ,  [0] 15 minute low ) + ( ( [-1] 15 minute max( 499 ,  [0] 15 minute high ) - [-1] 15 minute min( 499 ,  [0] 15 minute low ) ) * 0.9 ) | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 15 | Enabled | [-1] 15 minute max( 499 ,  [0] 15 minute high ) < [-500] 15 minute max( 500 ,  [0] 15 minute high ) | Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **#9** `daily high crossed above 1 day ago min( 499 ,  daily low ) + ( 1 day ago max( 499 ,  daily high ) - 1 day ago min( 499 ,  daily low ) ) * 0.9` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars.
- **#10** `1 day ago max( 499 ,  daily high ) < 500 days ago max( 500 ,  daily high )` — Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars.
...
- **#14** `[0] 15 minute high crossed above [-1] 15 minute min( 499 ,  [0] 15 minute low ) + ( ( [-1] 15 minute max( 499 ,  [0] 15 minute high ) - [-1] 15 minute min( 499 ,  [0] 15 minute low ) ) * 0.9 )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#15** `[-1] 15 minute max( 499 ,  [0] 15 minute high ) < [-500] 15 minute max( 500 ,  [0] 15 minute high )` — Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
### Measures observed
- `high` — appears 15 time(s) in the expression tree
...
- **Method context:** Mean reversion, Volatility, Breakout, Volume/delivery, Momentum, Multi-factor.
...
- **Horizon:** Intraday
- **Methods:** Mean reversion, Volatility, Breakout, Volume/delivery, Momentum, Multi-factor
- **Tags:** long-bias, universe:futures, indicator:volume, timeframe:intraday-bars, timeframe:daily
