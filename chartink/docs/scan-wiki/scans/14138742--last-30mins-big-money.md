...
This scan, titled "last 30mins big money", appears designed to screen Indian equities in the **cash** universe using **1 enabled** condition(s) combined with root join **all (AND)**.
...
|---:|---|---|---|
| 1 | Disabled | [0] 5 minute volume * [0] 5 minute close crossed above 200000000 | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Currently disabled in source — not applied when the scan runs. Volume condition gates participation/liquidity. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 2 | Enabled | [25] 15 minute sum( close ,  2 ) crossed above 900000000 | Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). Uses an intraday bar size (minute timeframe) rather than daily-only data. |
