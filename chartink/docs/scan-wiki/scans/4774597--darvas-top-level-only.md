...
- Chartink URL: https://chartink.com/screener/darvas-top-level-only
- Scan ID: `4774597`
- Slug: `darvas-top-level-only`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2021-06-02T10:41:38.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/4774597.json](../source-snapshots/4774597.json)
- Text snapshot: [source-snapshots/4774597.txt](../source-snapshots/4774597.txt)
...
Scan id: 4774597
Slug: darvas-top-level-only
Source URL: https://chartink.com/screener/darvas-top-level-only
...
3. [Enabled] daily count streak( 3, 1 where daily high < 1 day ago max( 5 ,  daily high ) ) >= 3
    group_path: root/group[cash|all]
4. [Enabled] [GROUP segment=cash join=all combination=passes measurevalue=default]  (path: root/group[cash|all])
5. [Enabled] [-4] 30 minute high crossed above [-4] 30 minute max( 500 ,  [-4] 30 minute high )
    group_path: root/group[cash|all]
...
- **#2** `4 days ago high crossed above 4 days ago max( 250 ,  4 days ago high )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars.
- **#3** `daily count streak( 3, 1 where daily high < 1 day ago max( 5 ,  daily high ) ) >= 3` — Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars.
- **#5** `[-4] 30 minute high crossed above [-4] 30 minute max( 500 ,  [-4] 30 minute high )` — Requires a bullish crossover event (left series moves from at/below to above the right series on the selected bar). max(N, series) is the highest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
- **#6** `[0] 30 minute count streak( 3, 1 where [0] 30 minute high < [-1] 30 minute max( 5 ,  [0] 30 minute high ) ) >= 3` — Inequality test: left expression must be strictly less than right. max(N, series) is the highest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data.
...
- Timeframe tokens: `0_days_ago`, `1_days_ago`, `30_minute`, `4_days_ago`
