...
classification: ["Mean reversion", "Momentum", "Price action", "Multi-factor"]
tags: ["short-bias", "universe:futures", "timeframe:intraday-bars", "timeframe:daily"]
...
This scan, titled "Price Change Short term retest low", appears designed to screen Indian equities in the **futures** universe using **1 enabled** condition(s) combined with root join **all (AND)**.

Dominant method tag(s) inferred from conditions: **Mean reversion, Momentum, Price action, Multi-factor**. Likely horizon label from name/timeframes: **Multi-horizon**.
...
|---:|---|---|---|
| 1 | Disabled | [0] 1 minute sum( close ,  300 ) crossed below -1 | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). Currently disabled in source — not applied when the scan runs. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 2 | Disabled | [0] 15 minute sum( close ,  20 ) < -4.5 | Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 3 | Enabled | [0] 15 minute sum( close ,  500 ) crossed below [-1] 15 minute min( 1000 ,  [0] 15 minute sum( close ,  500 ) ) * 0.9 | Requires a bearish crossover event (left series moves from at/above to below the right series on the selected bar). min(N, series) is the lowest value of series over N bars. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **Horizon:** Multi-horizon
- **Methods:** Mean reversion, Momentum, Price action, Multi-factor
- **Tags:** short-bias, universe:futures, timeframe:intraday-bars, timeframe:daily
