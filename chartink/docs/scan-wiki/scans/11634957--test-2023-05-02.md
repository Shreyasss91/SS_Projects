---
scan_id: 11634957
scan_name: test 2023-05-02
source_url: https://chartink.com/screener/test-2023-05-02-17
...
- Chartink URL: https://chartink.com/screener/test-2023-05-02-17
- Scan ID: `11634957`
- Slug: `test-2023-05-02-17`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Swing
- Created at (Chartink): 2023-05-02T12:58:07.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/11634957.json](../source-snapshots/11634957.json)
- Text snapshot: [source-snapshots/11634957.txt](../source-snapshots/11634957.txt)
...
- daily close > 50
- daily abs( daily close - daily open ) < ( ( daily high - daily low ) * 0.01 )
...
```text
Scan name: test 2023-05-02
Scan id: 11634957
Slug: test-2023-05-02-17
Source URL: https://chartink.com/screener/test-2023-05-02-17
Root universe/segment: cash
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2023-05-02T12:58:07.000000Z
...
4. [Enabled] daily close > 50
    group_path: root/group[cash|all]
5. [Disabled] daily abs( daily open - daily low ) / ( daily high - daily low ) > 0.4
6. [Enabled] daily abs( daily close - daily open ) < ( ( daily high - daily low ) * 0.01 )
7. [Disabled] daily true range( 1 ) > 1 day ago avg true range( 3 ) * 0.8
8. [Disabled] daily close > daily open
9. [Disabled] 1 day ago count( 3, 1 where daily abs( daily open - daily close ) < daily abs( 1 day ago open - 1 day ago close ) ) >= 3
10. [Disabled] daily count streak( 3, 1 where daily volume < 1 day ago volume ) = 3

=== Literal Chartink atlas_query (compiled active query; typically omits disabled filters) ===

( cash ( ( cash ( 1 day ago close * 1 day ago volume > 1000000000 * 0.1 and( latest avg true range( 7 ) / latest sma( latest close , 7 ) ) * 100 > 3 and latest close > 50 ) ) and abs( latest close - latest open ) < ( ( latest high - latest low ) * 0.01 ) ) )
```
...
| 4 | 5 | Disabled | root | daily abs( daily open - daily low ) / ( daily high - daily low ) > 0.4 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. |
| 5 | 6 | Enabled | root | daily abs( daily close - daily open ) < ( ( daily high - daily low ) * 0.01 ) | Inequality test: left expression must be strictly less than right. |
| 6 | 7 | Disabled | root | daily true range( 1 ) > 1 day ago avg true range( 3 ) * 0.8 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. ATR measures smoothed true range (volatility), not direction. |
| 7 | 8 | Disabled | root | daily close > daily open | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. |
| 8 | 9 | Disabled | root | 1 day ago count( 3, 1 where daily abs( daily open - daily close ) < daily abs( 1 day ago open - 1 day ago close ) ) >= 3 | Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. |
| 9 | 10 | Disabled | root | daily count streak( 3, 1 where daily volume < 1 day ago volume ) = 3 | Inequality test: left expression must be strictly less than right. Currently disabled in source — not applied when the scan runs. Volume condition gates participation/liquidity. |
...
- **#6** `daily abs( daily close - daily open ) < ( ( daily high - daily low ) * 0.01 )` — Inequality test: left expression must be strictly less than right.
...
### Disabled #5
- **Condition (verbatim):** `daily abs( daily open - daily low ) / ( daily high - daily low ) > 0.4`
...
### Disabled #7
- **Condition (verbatim):** `daily true range( 1 ) > 1 day ago avg true range( 3 ) * 0.8`
...
### Disabled #9
- **Condition (verbatim):** `1 day ago count( 3, 1 where daily abs( daily open - daily close ) < daily abs( 1 day ago open - 1 day ago close ) ) >= 3`
...
### Disabled #10
- **Condition (verbatim):** `daily count streak( 3, 1 where daily volume < 1 day ago volume ) = 3`
...
- `true range` — appears 1 time(s) in the expression tree
