...
horizon: Swing
classification: ["Fundamental", "Moving average", "Volume/delivery", "Breakout", "Multi-factor"]
...
# big vol_bigfiercyfightbearsandbulls_endbullswin_BIGBULLfollowsnextday

## Source

- Chartink URL: https://chartink.com/screener/big-vol-bigfiercyfightbearsandbulls-endbullswin-bigbullfollowsnextday
- Scan ID: `7455141`
- Slug: `big-vol-bigfiercyfightbearsandbulls-endbullswin-bigbullfollowsnextday`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Swing
- Created at (Chartink): 2022-01-11T04:41:14.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/7455141.json](../source-snapshots/7455141.json)
- Text snapshot: [source-snapshots/7455141.txt](../source-snapshots/7455141.txt)

## What this scan is for

This is a **swing** screen over **cash** with **7** active leaf condition(s) under root join **all (AND)**.
Its method labels are derived only from active expressions: **Fundamental, Moving average, Volume/delivery, Breakout, Multi-factor**.
...
Scan id: 7455141
Slug: big-vol-bigfiercyfightbearsandbulls-endbullswin-bigbullfollowsnextday
Source URL: https://chartink.com/screener/big-vol-bigfiercyfightbearsandbulls-endbullswin-bigbullfollowsnextday
...
3. [Enabled] daily market cap < 10000
    group_path: root/group[cash|all]
4. [Enabled] daily close > daily open
    group_path: root/group[cash|all]
5. [Enabled] daily high > daily close * 1.03
    group_path: root/group[cash|all]
6. [Enabled] daily volume > 1 day ago sma( close ,  5 ) * 3
    group_path: root/group[cash|all]
7. [Enabled] daily count( 90, 1 where daily volume > 1 day ago sma( close ,  5 ) * 3 ) > 1
    group_path: root/group[cash|all]
8. [Enabled] daily high > 1 day ago max( 75 ,  daily high ) * 0.95
    group_path: root/group[cash|all]
...
- **#4** `daily close > daily open` — Inequality test: left expression must be strictly greater than right.
- **#5** `daily high > daily close * 1.03` — Inequality test: left expression must be strictly greater than right.
- **#6** `daily volume > 1 day ago sma( close ,  5 ) * 3` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity.
- **#7** `daily count( 90, 1 where daily volume > 1 day ago sma( close ,  5 ) * 3 ) > 1` — Inequality test: left expression must be strictly greater than right. SMA is the arithmetic mean of the chosen field over N bars. Volume condition gates participation/liquidity.
- **#8** `daily high > 1 day ago max( 75 ,  daily high ) * 0.95` — Inequality test: left expression must be strictly greater than right. max(N, series) is the highest value of series over N bars.
...
### Disabled #9
- **Condition (verbatim):** `daily count( 240, 1 where daily close > 5 days ago close * 1.25 ) > 1`
...
- Timeframe tokens: `0_days_ago`, `1_days_ago`, `5_days_ago`

## How to use it

- **Horizon context:** treat as **Swing** unless live bar size usage suggests otherwise; confirm against the timeframe tokens in the definition.
- **Universe:** results are scoped to **cash**. Liquidity and index membership still vary inside that set.
- **Method context:** Fundamental, Moving average, Volume/delivery, Breakout, Multi-factor.
...
- **Horizon:** Swing
- **Methods:** Fundamental, Moving average, Volume/delivery, Breakout, Multi-factor
- **Tags:** bias:upward-condition, bias:downward-condition, universe:cash, indicator:volume, indicator:sma, timeframe:daily
- **Root universe:** cash
- **Root join:** all
