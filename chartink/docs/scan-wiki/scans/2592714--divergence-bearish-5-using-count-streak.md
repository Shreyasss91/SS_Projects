---
scan_id: 2592714
scan_name: "Divergence bearish 5% USING COUNT STREAK"
source_url: https://chartink.com/screener/count-streak-test
market: Indian equities
horizon: Intraday
classification: ["Oscillator", "Volume/delivery", "Momentum", "Multi-factor"]
tags: ["short-bias", "universe:futures", "indicator:rsi", "indicator:volume", "timeframe:intraday-bars", "timeframe:daily"]
...
# Divergence bearish 5% USING COUNT STREAK

## Source

- Chartink URL: https://chartink.com/screener/count-streak-test
- Scan ID: `2592714`
- Slug: `count-streak-test`
- Captured: 2026-07-15T12:56:06+05:30
- Market: Indian equities
- Intended horizon: Intraday
- Created at (Chartink): 2020-07-26T06:26:52.000000Z
- Private: False
- Favourite flag: 0
- Alert present flag: 0
- Raw snapshot: [source-snapshots/2592714.json](../source-snapshots/2592714.json)
- Text snapshot: [source-snapshots/2592714.txt](../source-snapshots/2592714.txt)

## What this scan is for

This scan, titled "Divergence bearish 5% USING COUNT STREAK", appears designed to screen Indian equities in the **futures** universe using **4 enabled** condition(s) combined with root join **all (AND)**.
...
Author description (source metadata): SAME AS https://chartink.com/screener/divergence-bearish-5-1
...
```text
Scan name: Divergence bearish 5% USING COUNT STREAK
Scan id: 2592714
Slug: count-streak-test
Source URL: https://chartink.com/screener/count-streak-test
Root universe/segment: futures
Root join: all (AND)
Root combination: passes
Root measurevalue: default
is_private: False
created_at: 2020-07-26T06:26:52.000000Z

=== Condition tree (from atlas_json; includes Enabled and Disabled) ===

1. [Enabled] 1 day ago close * 1 day ago volume > 1000000000
2. [Enabled] daily count streak( 5, 1 where daily close > 1 day ago close ) >= 5
3. [Enabled] daily count streak( 5, 1 where daily rsi( 14 ) > 1 day ago rsi( 14 ) ) >= 5
4. [Enabled] daily high > 1 day ago high * 1.05
5. [Disabled] [0] 15 minute count streak( 15, 1 where [0] 15 minute close > [-1] 15 minute close ) >= 10
6. [Disabled] [0] 15 minute count streak( 15, 1 where [0] 15 minute rsi( 14 ) > [-1] 15 minute rsi( 14 ) ) >= 10

=== Chartink atlas_query (compiled/active form; typically omits disabled filters) ===

( futures ( 1 day ago close * 1 day ago volume > 1000000000 and latest countstreak( 5, 1 where latest close > 1 day ago close ) >= 5 and latest countstreak( 5, 1 where latest rsi( 14 ) > 1 day ago rsi( 14 ) ) >= 5 and latest high > 1 day ago high * 1.05 ) )
```
...
| 2 | Enabled | daily count streak( 5, 1 where daily close > 1 day ago close ) >= 5 | Inequality test: left expression must be strictly greater than right. |
| 3 | Enabled | daily count streak( 5, 1 where daily rsi( 14 ) > 1 day ago rsi( 14 ) ) >= 5 | Inequality test: left expression must be strictly greater than right. RSI is a momentum oscillator from average gains/losses over its period. |
| 4 | Enabled | daily high > 1 day ago high * 1.05 | Inequality test: left expression must be strictly greater than right. |
| 5 | Disabled | [0] 15 minute count streak( 15, 1 where [0] 15 minute close > [-1] 15 minute close ) >= 10 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
| 6 | Disabled | [0] 15 minute count streak( 15, 1 where [0] 15 minute rsi( 14 ) > [-1] 15 minute rsi( 14 ) ) >= 10 | Inequality test: left expression must be strictly greater than right. Currently disabled in source — not applied when the scan runs. RSI is a momentum oscillator from average gains/losses over its period. Uses an intraday bar size (minute timeframe) rather than daily-only data. |
...
- **#2** `daily count streak( 5, 1 where daily close > 1 day ago close ) >= 5` — Inequality test: left expression must be strictly greater than right.
- **#3** `daily count streak( 5, 1 where daily rsi( 14 ) > 1 day ago rsi( 14 ) ) >= 5` — Inequality test: left expression must be strictly greater than right. RSI is a momentum oscillator from average gains/losses over its period.
- **#4** `daily high > 1 day ago high * 1.05` — Inequality test: left expression must be strictly greater than right.
...
### Disabled #5
- **Condition (verbatim):** `[0] 15 minute count streak( 15, 1 where [0] 15 minute close > [-1] 15 minute close ) >= 10`
...
### Disabled #6
- **Condition (verbatim):** `[0] 15 minute count streak( 15, 1 where [0] 15 minute rsi( 14 ) > [-1] 15 minute rsi( 14 ) ) >= 10`
