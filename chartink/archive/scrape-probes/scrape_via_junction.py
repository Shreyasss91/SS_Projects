"""
Use a directory junction to the real Chrome User Data so remote-debugging-port
is allowed (custom --user-data-dir) while still using the real session files.
"""
...
def make_junction() -> Path:
    link = Path(tempfile.mkdtemp(prefix="chrome_junc_parent_")) / "UserData"
    # mklink /J needs cmd
    r = subprocess.run(
        ["cmd", "/c", "mklink", "/J", str(link), str(SRC_USER_DATA)],
        capture_output=True,
        text=True,
    )
    print(r.stdout)
    ...
    if r.returncode != 0 or not link.exists():
        raise RuntimeError(f"mklink failed: {r.returncode} {r.stdout} {r.stderr}")
    return link


def wait_port(port: int, timeout: float = 50.0) -> dict:
    ...
    while time.time() < deadline:
        try:
            ...
            last = e
            time.sleep(0.4)
    raise RuntimeError(f"port not ready: {last}")
...
def fetch_via_browser(driver) -> list[dict]:
    ...
    for page in range(1, PAGES + 1):
        ...
        print(f"browser page {page}: {driver.current_url} len={len(html)}")
        (RAW / f"browser_page_{page}.html").write_text(html, encoding="utf-8")
        if not is_app_url(driver.current_url or ""):
            raise RuntimeError(f"not on dashboard: {driver.current_url}")
...
def main() -> None:
    ...
    try:
        ...
        (RAW / "junction_p1.html").write_text(driver.page_source, encoding="utf-8")

        if not is_app_url(driver.current_url or ""):
            print("NOT logged in via junction profile. Falling back to browser wait...")
            raise RuntimeError("not authenticated via junction")
        ...
        try:
            ...
            pass
        # do not force-kill if user wants browser; but cleanup for automation
        subprocess.run(["taskkill", "/F", "/IM", "chrome.exe"], capture_output=True)
        # remove junction only (not target)
        try:
            link.rmdir()
        except Exception as e:
            print("junction remove:", e)
