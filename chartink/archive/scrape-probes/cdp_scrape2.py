"""
Copy a minimal Chrome profile (with Cookies), launch with remote debugging,
pull Chartink session via CDP/Selenium, scrape dashboard, write index.
"""
...
SRC_USER_DATA = Path.home() / "AppData/Local/Google/Chrome/User Data"
DEBUG_PORT = 9333
UA = (
...
def copy_minimal_profile(dst_user_data: Path) -> None:
    dst_user_data.mkdir(parents=True, exist_ok=True)
    # Local State (encryption keys)
    shutil.copy2(SRC_USER_DATA / "Local State", dst_user_data / "Local State")

    src_def = SRC_USER_DATA / "Default"
    dst_def = dst_user_data / "Default"
    dst_def.mkdir(parents=True, exist_ok=True)
    (dst_def / "Network").mkdir(parents=True, exist_ok=True)
    ...
    for rel, required in files:
        ...
        print(f"copied {rel} ({dst.stat().st_size} bytes)")

    # First Run sentinel so chrome doesn't block
    (dst_user_data / "First Run").write_text("", encoding="utf-8")


def wait_port(port: int, timeout: float = 45.0) -> dict:
...
def launch_chrome(user_data: Path) -> subprocess.Popen:
    args = [
        str(CHROME),
...
def cookies_via_selenium() -> dict[str, str]:
    ...
    (RAW / "selenium_p1.html").write_text(driver.page_source, encoding="utf-8")
    out = {}
...
def fetch_all(session: requests.Session) -> list[dict]:
    ...
    for page in range(1, PAGES + 1):
        ...
        print(
            f"page {page}: {r.status_code} final={r.url} "
            f"ct={r.headers.get('content-type')} len={len(r.text)}"
        )
        (RAW / f"dash_page_{page}.raw").write_text(r.text[:500000], encoding="utf-8")
        if "login" in r.url.lower():
            # try HTML without inertia
            r = session.get(url, headers={"User-Agent": UA}, timeout=60)
            print(f"  retry html {r.status_code} {r.url} len={len(r.text)}")
            (RAW / f"dash_page_{page}_html.raw").write_text(r.text[:500000], encoding="utf-8")
            if "login" in r.url.lower():
        ...
        (RAW / f"dash_page_{page}.json").write_text(
            json.dumps(data, indent=2, default=str)[:1000000], encoding="utf-8"
        )
        scans, meta = extract_scans(data.get("props") or {})
        print(f"  scans={len(scans)} meta={meta}")
        if not scans:
            raise RuntimeError(f"no scans page {page}")
...
def main() -> None:
    print("Killing Chrome to unlock profile files...")
    kill_chrome()

    tmp = Path(tempfile.mkdtemp(prefix="chrome_min_"))
    print("temp profile", tmp)
    copy_minimal_profile(tmp)

    proc = launch_chrome(tmp)
    try:
        ver = wait_port(DEBUG_PORT, timeout=50)
        print("Chrome CDP ready:", ver.get("Browser"))
        time.sleep(3)
        cookies = cookies_via_selenium()
        if not cookies:
            raise RuntimeError("No chartink cookies from selenium — login may be required")
        ...
        # leave chrome open for user? kill temp chrome
        try:
            proc.terminate()
        except Exception:
            pass
